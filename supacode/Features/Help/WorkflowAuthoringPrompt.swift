...
/// manual, and asks it to author, validate, and place a workflow file. Localized like
  ...
  private static func english(skill: String, manual: String, directory: String) -> AskAgentHelpStrings {
    AskAgentHelpStrings(
      ...
      explanation:
        ...
        + "write, validate, and place a workflow file for you.",
      prompt: """
        Write a Prowl Agent Workflow for me.

        Prowl runs multi-agent workflows from YAML files (schema prowl.workflow/v1). The authoring \
        guide and the feature manual ship inside the app:
        ...
        and when it ends.
        2. Write the file into \(directory) with a short, unique id.
        3. Validate it with `prowl workflow validate <file>` and fix every error.
  ...
  private static func simplifiedChinese(skill: String, manual: String, directory: String) -> AskAgentHelpStrings {
    AskAgentHelpStrings(
      ...
      explanation:
        ...
        + "它会让 agent 读取 Prowl 内置的 workflow skill 和说明书，然后替你编写、校验并放置 workflow 文件。",
      prompt: """
        请为我编写一个 Prowl Agent Workflow。

        Prowl 用 YAML 文件（schema prowl.workflow/v1）运行多 agent 工作流。编写指南和功能说明书都打包在 app 内：
        ...
        2. 用一个简短且唯一的 id，把文件写到 \(directory)。
        3. 用 `prowl workflow validate <文件>` 校验，并修复所有错误。
  ...
  private static func traditionalChinese(skill: String, manual: String, directory: String) -> AskAgentHelpStrings {
    AskAgentHelpStrings(
      ...
      explanation:
        ...
        + "它會讓 agent 讀取 Prowl 內建的 workflow skill 和說明書，然後替你撰寫、檢驗並放置 workflow 檔案。",
      prompt: """
        請為我撰寫一個 Prowl Agent Workflow。

        Prowl 用 YAML 檔案（schema prowl.workflow/v1）執行多 agent 工作流。撰寫指南和功能說明書都打包在 app 內：
        ...
        2. 用一個簡短且唯一的 id，把檔案寫到 \(directory)。
        3. 用 `prowl workflow validate <檔案>` 檢驗，並修正所有錯誤。
  ...
  private static func japanese(skill: String, manual: String, directory: String) -> AskAgentHelpStrings {
    AskAgentHelpStrings(
      ...
      explanation:
        ...
        + "ワークフローファイルの作成・検証・配置を任せられます。",
      prompt: """
        Prowl の Agent Workflow を書いてください。

        Prowl は YAML ファイル（schema prowl.workflow/v1）で複数エージェントのワークフローを実行します。\
        ...
        2. 短くて一意な id を付けて、ファイルを \(directory) に書いてください。
        3. `prowl workflow validate <ファイル>` で検証し、すべてのエラーを修正してください。
