...
nonisolated enum MirrorRelay {
  ...
  private static func run(port: UInt16, token: String) throws {
    ...
    try writeAll(
      try MirrorWire.encode(MirrorMessage(kind: .list, bytes: Data(token.utf8))), to: descriptor)
    var polls = [
    ...
    while true {
      ...
      if polls[0].revents & Int16(POLLIN) != 0 {
        ...
        guard message.kind == .frame, let frame = message.frame, let sequence = message.sequence
        else {
          throw MirrorProtocolError.invalidMessage
        }
        ...
        try writeAll(
          try MirrorWire.encode(
            MirrorMessage(
              version: version, kind: .acknowledge,
              sequence: sequence, subscriptionID: subscriptionID)), to: descriptor)
      }
      if polls[1].revents & Int16(POLLIN) != 0 {
        ...
        try writeAll(
          try MirrorWire.encode(
            MirrorMessage(
              version: version, kind: .input, bytes: Data(bytes.prefix(readCount)),
              subscriptionID: subscriptionID)), to: descriptor)
      }
