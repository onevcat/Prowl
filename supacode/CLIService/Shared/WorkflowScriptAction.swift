...
nonisolated public struct WorkflowScriptAction: Equatable, Sendable {
  ...
  public static func parse(_ yaml: String, id: String, files: [String: Data] = [:]) throws -> Self {
    ...
    guard root["schema"] == .string("prowl.action/v1"), WorkflowSchema.isSlug(id),
    ...
    try checkKeys(backend.keys, allowed: ["type", "interpreter", "entrypoint", "arguments", "environment"])
    ...
    let environment = try strings(backend["environment"])
    ...
    return Self(
      ...
      environment: environment, timeoutSeconds: timeout, inputSchema: input, outputSchema: output,
