...
| Remote Mirror | `supacode/Features/RemoteMirror/RemoteMirrorPaneView.swift` | Navigation Host button with native title/subtitle, principal connection status matching local status styling, primary action group for recovery, display size, history and disconnect. | Native window toolbar; only display size opens a menu. |
