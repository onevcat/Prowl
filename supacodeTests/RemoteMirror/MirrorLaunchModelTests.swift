...
struct MirrorLaunchModelTests {
  ...
  @Test func lostCreationResponseIsNeverAutomaticallyReplayed() async {
    ...
    #expect(!model.canCreate)
    let warning = model.error
    await model.load()
    #expect(model.error == warning)
    #expect(await model.create() == nil)
    #expect(calls == 1)
  }
  ...
  @Test func shellCreationUsesDormantWorktreesAndNeedsNoProfileOrPrompt() async throws {
    var captured: MirrorCommandRequest.Create?
    let model = MirrorLaunchModel(supportsShell: true) { command in
      switch command {
      case .list:
        return try json(
          #"""
      ...
      case .profiles: return try json(#"{"ok":true,"data":{"profiles":[]}}"#)
    ...
    await model.load()
    #expect(model.worktreeID == "dormant")
    #expect(model.canCreate)
    model.prompt = "Do not send this to the shell"
    #expect(await model.create() != nil)
    #expect(captured?.launch == nil)
    #expect(captured?.background == true)
    model.kind = .agent
    #expect(!model.canCreate)
  }

  @Test func catalogFailureClearsStaleLaunchSelections() async {
    var fails = false
    let model = MirrorLaunchModel { command in
    ...
    #"""
    {
      ...
      "data": {
        "target": {
          ...
          "pane": {
            "id": "03F5535A-D08C-498C-B70B-C665AC8D02CB",
            "title": "Codex"
          }
        }
      }
